...
def submit_idea(redis_conn):
    ...
    if similar_ideas:
        print('Warning: The submitted idea is similar to the following existing ideas (use /quit to exit):')
    ...
    while True:
        names = input('Enter the name(s) of the person(s) who made the idea (comma-separated) (use /quit to exit): ')
        if names.strip() == '':
